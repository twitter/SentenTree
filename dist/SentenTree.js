(function (global, factory) {
    ...
    (factory((global.SentenTree = global.SentenTree || {})));
...
var DEFAULT_STOP_WORDS = uniq(NLTK_STOP_WORDS.concat(CUSTOM_STOP_WORDS).concat(TWEET_STOP_WORDS));
...
var WordFilter = function () {
  function WordFilter() {
    ...
    classCallCheck(this, WordFilter);
    ...
    if (excludeWords && excludeWords.length > 0) {
      (function () {
        var exclusionLookup = keyBy(excludeWords, function (w) {
          return w;
        });
...
var defaultNodeCnt = 150;

function initGraphs(tokens, terms) {
...
function expandSeqTree(rootSeq, graphs, expandCnt, minSupport, maxSupport, terms, itemset) {
  ...
  while (seqs.length > 0 && expandCnt > 0) {
    ...
    if (s1) seqs.push(s1);
    if (s0 && s0.size >= minSupport) seqs.push(s0);
  }
...
function growSeq(seq, terms, minSupport, maxSupport, itemset) {
  ...
  for (var s = 0; s <= seq.words.length; s++) {
    ...
    seq.DBs.forEach(function (t) {
      var l, r;
      if (s == 0) l = 0;else l = t.seqIndices[s - 1] + 1;
      if (s == seq.words.length) r = t.text.length;else r = t.seqIndices[s];
      var duplicate = [];
      for (var i = l; i < r; i++) {
        var w = t.text[i];
        if (w in duplicate) continue;else duplicate.push(w);
        if (w in fdist) fdist[w] += t.cnt;else fdist[w] = t.cnt;
      }
    });
    var maxw = null,
        maxc = 0;
    for (var w in fdist) {
      if (fdist[w] < maxSupport && fdist[w] > maxc && (!itemset[w].startsWith('#') || seq.words.length > 0) // no hashtag as root of tree
      ) {
  ...
  if (cnt >= minSupport) {
    ...
    for (var ti = 0; ti < seq.DBs.length; ti++) {
      var t = seq.DBs[ti];
      var l, r;
      if (pos == 0) l = 0;else l = t.seqIndices[pos - 1] + 1;
      if (pos == words.length) r = t.text.length;else r = t.seqIndices[pos];
  ...
  leafSeqs.forEach(function (seq) {
    if (graphs.indexOf(seq.graph) >= 0) {
      ...
      for (var i = 0; i < words.length - 1; i++) {
        ...
        if (!(words[i].nid in linkadj)) linkadj[words[i].nid] = {};
        if (words[i + 1].nid in linkadj[words[i].nid]) linkadj[words[i].nid][words[i + 1].nid] += seq.size;else linkadj[words[i].nid][words[i + 1].nid] = seq.size;
      ...
      for (var i = 0; i < words.length; i++) {
        if (!words[i].leafSeq || words[i].leafSeq.size < seq.size) words[i].leafSeq = seq;
      }
    }
  });
  console.log(freqMax);
...
var TokenizedDataset = function () {
  function TokenizedDataset() {
    ...
    classCallCheck(this, TokenizedDataset);
...
var DEFAULT_FILTER = new WordFilter();

var SentenTreeModel = function SentenTreeModel(entries) {
  var wordFilter = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : DEFAULT_FILTER;
  var termWeights = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
  classCallCheck(this, SentenTreeModel);

  var dataset = tokenize(entries).filter(wordFilter);
  var terms = dataset.encodeTermWeights(termWeights);
  initGraphs(dataset, terms);
};

exports.SentenTreeModel = SentenTreeModel;

Object.defineProperty(exports, '__esModule', { value: true });

})));
//# sourceMappingURL=SentenTree.js.map
